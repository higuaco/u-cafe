...
$body = trim( $body );
// echo $subject.$name.$ruby.$tel.$email.$email_check.$body;
//エラーメッセージを保存する配列の初期化
$error = array();
...
if (preg_match('/\A[[:^cntrl:]]{1,30}\z/u', $ruby) == 0) {
    $error[ 'ruby' ] = '*フリガナは30文字以内でお願いします。';
}
if ( $email == '' ) {
...
if ($tel == '') {
    $error[ 'tel' ] = '*電話番号は必須項目です。';
}else if ( preg_match( '/\A[[:^cntrl:]]{0,30}\z/u', $tel ) == 0 ) {
...
<body>
  ...
  <scrip src="https://www.google.com/recaptcha/api.js?render=<?php echo $siteKey; ?>">
    </script>
      ...
      $("#complete").submit(function(event) {
        ...
        grecaptcha.ready(function() {
          grecaptcha.execute('<?php echo $siteKey; ?>', {
            ...
            that.prepend('<input type="hidden" name="g-recaptcha-response" value="5">');
